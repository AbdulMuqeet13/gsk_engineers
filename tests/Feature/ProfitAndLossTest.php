...
class ProfitAndLossTest extends TestCase
{
    ...
    {
        $this->get(route('reports.profit-and-loss'))
    ...
    public function test_requires_financial_report_permission(): void
    {
        ...
        $this->actingAs($user)
            ->get(route('reports.profit-and-loss'))
            ->assertForbidden();
    }

    public function test_displays_income_and_expenses(): void
    {
        // Income: credit income, debit cash
        ...
        $response->assertInertia(fn ($page) => $page
            ->component('reports/profit-and-loss')
            ->has('incomeAccounts', 1)
            ->has('expenseAccounts', 1)
        );
    }

    public function test_calculates_net_profit(): void
    {
        $this->postEntry('2026-09-01', $this->cashAccount->id, $this->incomeAccount->id, '80000.00');
        $this->postEntry('2026-09-01', $this->expenseAccount->id, $this->cashAccount->id, '30000.00');
        ...
        $response->assertInertia(fn ($page) => $page
            ->where('totalIncome', '80000.00')
            ->where('totalExpenses', '30000.00')
            ->where('netProfit', '50000.00')
        );
    }
    ...
    public function test_filters_by_project(): void
    {
        ...
        $response = $this->actingAs($this->user)
            ->get(route('reports.profit-and-loss', ['project_id' => $project->id]));
